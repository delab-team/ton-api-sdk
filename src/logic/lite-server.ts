...
export class LiteServer {
  ...
  async getRawBlockchainBlock(block_id: string): Promise<GetRawBlockchainBlock | undefined> {
    const data = await this.tonApi.get('/v2/liteserver/get_block', { block_id })
  ...
  async getListBlockTransactions (block_id: string, mode: number, count: number, account_id?: string, lt?: number): Promise<GetListBlockTransactionsDto | undefined> {
    const data = await this.tonApi.get('liteserver/list_block_transactions', { block_id, mode, count, account_id, lt })

    console.log(data)
    return data
  }

  async getBlockProof (known_block: string, target_block: string, mode?: number): Promise<GetBlockProofDto | undefined> {
    const data = await this.tonApi.get('liteserver/get_block_proof', { known_block, target_block, mode })

    console.log(data)
    return data
  }

  async getConfigAll (block_id: string, mode: string): Promise<GetConfigAllDto | undefined> {
